...
export function BudgetItemsBreakdown({ byBudgetItem }: Props) {
  const _maxBudget = Math.max(...byBudgetItem.map((i) => i.budgetTHB), 1);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Budget and actual spending per expense category (THB)</CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        {byBudgetItem.map((item, _index) => {
          const utilizationPct = item.budgetTHB > 0 ? Math.min((item.actualTHB / item.budgetTHB) * 100, 100) : 0;
          const isOverBudget = Number(Math.abs(item.actualTHB)) > item.budgetTHB;

          return (
            <div key={item.budgetItemName} className="space-y-1">
              <div className="flex items-center justify-between text-sm">
                <div>
                  <span className="truncate font-medium">{item.budgetItemName}</span>
                </div>
                <span
                  className={`shrink-0 text-xs tabular-nums ${isOverBudget ? "font-semibold text-destructive" : "text-muted-foreground"}`}
                >
                  {item.budgetTHB > 0 ? `${((item.actualTHB / item.budgetTHB) * 100).toFixed(0)}%` : "—"}
                </span>
              </div>
