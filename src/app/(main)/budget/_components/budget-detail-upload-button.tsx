...
export function BudgetDetailUploadButton({ onSuccess }: Props) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const res = await fetch("/api/budget-detail", { method: "POST", body: formData });
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <Button variant="outline" disabled={status === "loading"} onClick={() => inputRef.current?.click()}>
        {status === "loading" ? "Importing..." : "Import Budget Detail CSV"}
      </Button>
      {message && (
